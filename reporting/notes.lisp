                      ...
                      ,(lambda (r)
                         (let ((err-text (fail-condition-text r)))
                           (when (or ;; The symbol NON-PROPAGATING-OPERATION is not present in package ASDF/INTERFACE.
                                     ...
                                     (search "unknown &KEY argument: :AT" err-text)
                                     (search "Incorrect keyword arguments in (:AT" err-text)
                                     (search "Unknown keyword :AT" err-text)
                                     (search "You need ASDF >= 3.1 to load this system correctly." err-text)
                                     (ppcre:scan (load-time-value (ppcre:create-scanner "Invalid initialization arguments:\\w*:SOURCE-CONTROL,\\w*:BUG-TRACKER,\\w*:HOMEPAGE\\w*in call for class #<STANDARD-CLASS SYSTEM>"))
                                                 err-text))
                             "needs newer ASDF"))))))))
