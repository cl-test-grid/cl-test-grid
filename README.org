...
* The Goal
  ...
  And other way around - failed tests not always
  means the library is broken - it may be just 
  a bug in the tests themselves. 

  Redusing this gap increases the utility of automated testing.
...
* Status
  ...
  of the CL libraries using quicklisp to download the libraries
  and uploads the test results to the central server.

  Current we have added 23 [[http://blog.quicklisp.org/2010/11/project-download-statistics.html][most often quicklisp-downloaded libraries]].

  File [[run.sh.sample]] shows what parameters to use for particular
  ...
  Test results are published as reports here:
...
** Discussing the project
   ...
   - Distribuing the work. Currently we considered
     ...
     [Today hhis featus has quite low priority]
