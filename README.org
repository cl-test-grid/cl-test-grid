...
** Running tests
   ...
   System requirements: as of today, only two lisps are
   known to run test-grid-agent successfully: CCL and SBCL.
   Test-grid-agent uses number of libraries, therefore
   we recommend to have Quicklisp installed and
   added to the CCL or SBCL init file.
   ...
   6. Edit the run-agent.lisp (specify all the lisp implementations you have, and some other settings)
   7. ./run-agent.sh
