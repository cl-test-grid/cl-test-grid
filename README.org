...
[[http://common-lisp.net/project/cl-test-grid/][Test Results]] | [[https://bugs.launchpad.net/common-lisp][Bugs Found]] ([[docs/issues-found.org][Old Bug Registry]]).
...
* Status
  ...
  it with a list if CL implementations installed on
  his machine, and test-grid-agent runs tests on these
  implenentations, starting each test suite in a separate lisp 
  process.

  Test-grid-agent may be run manually from command line or
  configured as a periodical task with cron or similary service.
  ...
  published as lisp data file and as HTML reports here:
...
** Running tests
   ...
   4. Edit the run-agent.sh (edit one line).
   ...
   run tests and upload the results.

   Agent keeps log files in the cl-test-grid/work-dir/agent/logs,
   where you may control what it has done.
...
#+END_SRC
   ...
   Feel free contact us if you have any questions or difficulties
