...
** Running tests
   ...
   Agent starts from checking the configuraion - if it can run all the lisp
   implementations configured. Then preforms tests if necessary,
   uploads results and exits.

   Next time all you need is to just start ./run-agent.sh. 

   As you may noticed, run-agent.sh updates cl-test-grid from git
   so that we always run the lates version.
