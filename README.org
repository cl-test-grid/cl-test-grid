...
* Donate
  ...
  If you want to fund a particular goal, it should be negotiated first.

* Credits
 - Anton Vodonosov
   Project author, developer and maintainer and test runner.
 - Juan Jose Garcia-Ripoll
   His ECL test farm was one of the inspirations for the project.
   Also Juan Jose provided some very valuable feedback and suggestions about the project.
 - Paul Nathan
   Maintains Linux and Mac OSX build machines and constantly contributes test results for various Lisps.
 - Robert P. Goldman
   For moral support and running tests in the early stage of the project.
 - Erik Huelsmann and [[efficito.com]]
   For hosting a Linux VM to run our tests.
 - Mark Safronov
   Contributes test results.
 - Dave Cooper and [[http://genworks.com/][Genworks International]]
   For donation
 - [[http://www.cl-foundation.org/][Common Lisp Fundation]]
   For handling donations
 - Daniel Zaru
   Implemented some code for retrieving test results from email box
   and the DB printing.
