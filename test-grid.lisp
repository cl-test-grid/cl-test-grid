    ...
    (dotimes (header-row-num col-field-count)
      ...
      (dolist (col-addr cols)
        (let* ((cell-addr (subseq col-addr 0 (1+ header-row-num)))
          ...
          (when (not (printed helper))
            (format out "<td colspan=\"~A\" class=nHeader>~A</td>" (span helper) (car (last cell-addr)))
            (setf (printed helper) t))))
      (format out "</tr>~%"))))
  ...
  (let (rows
    ...
    (let ((row-spans (calc-spans rows))
          (index-key (make-sequence 'list (+ (length row-fields)
                                             (length col-fields)))))
      (princ "<title>CL Test Grid Pivot Report</title>" out)
      (princ "<link href=../style.css rel=stylesheet />" out)
      (dolist (row rows)
        (princ "<tr class=nHeader>" out)
        ...
        (format out "</tr>~%"))))
  (princ "</table>" out)
  (princ "<a href=reports-overview.html class=link>To reports overview</a>" out)
