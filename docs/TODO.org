...
** Test loading of every ASDF system and publish results.
  <in progress>
...
** Workaround the GAE issue https://code.google.com/p/googleappengine/issues/detail?id=8032

* 0.5.2 Don't loose test run results if it was interrupted (due to errors, network outage or agent termination)
** Continuable test runs
* 0.5.3 Install more lisps on my VPS
...
* 0.5.4 Reuild free the lisps from source control daily, before running test-grid-agent
