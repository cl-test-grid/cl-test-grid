...
** DONE finish breaking-up the old monolitic module test-grid:
...
* 0.4.3 Separate status value for compilation failure
...
* 0.4.4 Test all the libraries in quicklisp. (if test-grid-testsuites-libtest is not implemented test compilation)
