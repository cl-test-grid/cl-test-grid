...
** TODO recover from submit-results failure
   ...
   upload fails, agent retarted next time tries to repeate full test run.
   It's because the test-run-info.lisp is not considered uninished - it already has log IDs.
...
** TODO Do not copy/paste implementation-identifier code from ASDF?
   And somehow just use the recent ASDF?
** TODO Do not copy/paste the print-backtrace from ASDF?
   And somehow just use the recent ASDF?
