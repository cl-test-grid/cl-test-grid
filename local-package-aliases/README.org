...
#+BEGIN_SRC lisp
  ...
  ;; $ in the middle of a token has not special
  ;; effect.
  (in-package :some-application)
  ...
  ;; is not aliases in the current package, then
  ;; $ is interpreted as usually:

  (local-package-aliases:set)
  ...
  ;; globally, for example from init file,
...
** Conclusion
   ...
   Advantabe of local-package-aliases is that it may be enabled
   once and forewer in the Lisp initialization file, while
   packag-renaming will require you to manually rename packages
   every time you switch projects or subsystems. On the other
   hand, if you renamed packages during Lisp session, SLIME
