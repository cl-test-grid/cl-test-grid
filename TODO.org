...
** TODO new status, :expected-failure (or better :know-failure).
   Also maybe :unexpected-ok.
** TODO Test status should include separate indicator for the library compilationa and loading.
** TODO ABCL, cffi testts: return :no-resource if JNA is not available
** TODO move closed TODO items to another (archive) file, so that the
