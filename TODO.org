...
** TODO For all the libraries which need manual configuration
   ...
   the tests.
** TODO test-run-info.lisp as attachment, instead of letter body     :server:
...
** TODO When the settings file is populated, inform user where     :settings:
...
* Backlog
** TODO An utility to delete blobs not used in db.lisp from the blobstore
** TODO Prevent HTML injection via test-run-info
